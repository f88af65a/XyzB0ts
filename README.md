...
### 通过类型
 参考plugins.rechat  
 通过listenType直接赋值、listenType.append或者addTypeToList函数在listenType中添加监听信息  
...
### 通过关键字
 参考plugins.saucenao  
 通过listenTarget直接赋值、listenTarget.append或者addTargetToList函数在listenTarget中添加监听信息  
 具体格式请参考注释
...
### 过滤器
 参考plugins.blacklist  
 通过filterList直接赋值、filterList.append或者addFilterToList函数在filterList中添加监听信息  
