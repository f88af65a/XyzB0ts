...
class plugin(BotPlugin):
    ...
    async def getMc(self, request: BotRequest):
        ...
        serverPort = 25565
        if len(data) < 2:
            await request.sendMessage(MessageChain().plain("缺少参数\n/mc ip [端口]不写默认25565"))
            return
        if len(data) >= 2:
            serverIp = data[1]
        if len(data) == 3:
            if not (data[2].isnumeric() and int(data[2]) >= 0 and int(data[2]) <= 65535):
        ...
        with socket.socket(socket.AF_INET,socket.SOCK_STREAM) as sock:
            ...
            while time.time() - stime <= 2 and breakFlag:
                ...
                if len(rdata) == 0:
                    await request.sendMessage(MessageChain().plain("接受请求时连接断开"))
                    return -1
                responseData += rdata
            ...
            if "extra" in responseData["description"]:
                for i in responseData["description"]["extra"]:
                    if "text" in i:
                        description += i["text"]
            try:
                printData = "信息:{0}\n版本:{1}\n人数:{2}/{3}".format(description,responseData["version"]["name"],responseData["players"]["online"],responseData["players"]["max"])
                if "playerlist" in data:
                    printData += "\n在线玩家:\n"
                    for i in range(0,len(responseData["players"]["sample"])):
                        printData += responseData["players"]["sample"][i]["name"]
    ...
    async def getBe(self, request):
        ...
        with socket.socket(socket.AF_INET,socket.SOCK_DGRAM) as sock:
            ...
            try:
                ...
                printData += f"版本:{responseData[0]} {responseData[2]} {responseData[3]}"
                await request.sendMessage(MessageChain().plain(printData))
            except Exception as e:
                await request.sendMessage(MessageChain().plain("解析过程中出错"))
                printTraceBack()
