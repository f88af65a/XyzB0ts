...
class plugin(BotPlugin):
    ...
    async def unSearch(self, request: BotRequest):
        ...
        if len(data) < 2:
            await request.sendMessage(MessageChain().plain(self.unSearch.__doc__))
            return
        searchData = b"\xFF\xFF\xFF\xFF\x54\x53\x6F\x75\x72\x63\x65\x20\x45\x6E\x67\x69\x6E\x65\x20\x51\x75\x65\x72\x79\x00"
